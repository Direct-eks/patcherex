    ...
    with open(filename, "rb") as f:
        ...
        if start_bytes.startswith(b"\x7fCGC"):
            ...
            if start_bytes.startswith(b"\x03\x00", 0x12):  # EM_386
                detourbackendclass = getattr(importlib.import_module("patcherex.arch.i386.detourbackend"),
                                             "DetourBackend")
            elif start_bytes.startswith(b"\xb7\x00", 0x12):  # EM_AARCH64
                detourbackendclass = getattr(importlib.import_module("patcherex.arch.aarch64.detourbackend"),
                                             "DetourBackend")
            elif start_bytes.startswith(b"\x28\x00", 0x12):  # EM_ARM
                detourbackendclass = getattr(importlib.import_module("patcherex.arch.arm.detourbackend"),
                                             "DetourBackend")
            elif start_bytes.startswith(b"\x08\x00", 0x12):  # EM_MIPS
                detourbackendclass = getattr(importlib.import_module("patcherex.arch.mips.detourbackend"),
                                             "DetourBackend")
            else:
