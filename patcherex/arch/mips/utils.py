...
def disassemble(code, offset=0x0, is_64=False):
    md = capstone.Cs(capstone.CS_ARCH_MIPS, capstone.CS_MODE_MIPS64 if is_64 else capstone.CS_MODE_MIPS32)
    md.detail = True
...
def compile_c(code, optimization='-Oz', name_map=None, compiler_flags="-m32", is_thumb=False):
    ...
    with tempdir() as td:
        ...
        res = exec_cmd("clang -nostdlib -mno-sse -target arm-linux-gnueabihf -ffreestanding %s %s -o %s -c %s %s" \
                       % ("-mthumb" if is_thumb else "", optimization, object_fname, c_fname, compiler_flags),
                       shell=True)
        if res[2] != 0:
            print("CLang error:")
            print(res[0])
