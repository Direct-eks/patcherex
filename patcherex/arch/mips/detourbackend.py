...
class DetourBackend(Backend):
    ...
    def setup_headers(self, segments):
        ...
        for segment in segments:
            if segment["p_type"] == "PT_LOAD":
                ...
                rounded_segments.append((
                    ...
                    ((segment["p_vaddr"] - self.first_load["p_vaddr"]) % 0x1000),
                    # end of the segment, round up to multiple of 0x1000
                    int((segment["p_vaddr"] + segment["p_memsz"] -
                         self.first_load["p_vaddr"] + 0x1000 - 1) / 0x1000 * 0x1000)
                ))
        ...
        for segment in segments:
            if segment["p_type"] == "PT_PHDR":
                ...
                rounded_segments = sorted(rounded_segments, key=lambda x: x[0])

                while True:
                    new_rounded_segments = []
                    i = 0
                    while i < len(rounded_segments) - 1:
                        current_segment = rounded_segments[i]
                        next_segment = rounded_segments[i + 1]
                        if current_segment[1] > next_segment[0]:  # two segments overlap
                            # append a combination of two segments
                            new_rounded_segments.append((current_segment[0], next_segment[1]))
                            i += 2
                        else:
                            # append segment without overlapping
                            new_rounded_segments.append(current_segment)
                            i += 1
                    if i == len(rounded_segments) - 1:
                        # append the last segment if without overlapping
                        new_rounded_segments.append(rounded_segments[i])
                    if new_rounded_segments == rounded_segments:
                        break
                    rounded_segments = new_rounded_segments  # combined segments, run again

                for prev_seg, next_seg in zip(rounded_segments[:-1], rounded_segments[1:]):
                    ...
                    self.phdr_start = rounded_segments[-1][1]  # otherwise put it after the last load segment
    ...
    def restore_state(self, applied_patches, removed_patches):
        ...
        if len(removed_patches) > 0:
            cut = min([len(applied_patches)] +
                      [applied_patches.index(p) for p in removed_patches if p in applied_patches])
    ...
    def apply_patches(self, patches):
        ...
        # With this backend here we can fail applying a patch, in case, resolve dependencies
        insert_code_patches = [p for p in patches if isinstance(p, InsertCodePatch)]
        ...
        if any([isinstance(p, ins) for ins in header_patches for p in self.added_patches]) or \
            ...
            if len(segment_header_patches) > 1:
                msg = "more than one patch tries to change segment headers: " + "|".join(
    ...
    def insert_detour(self, patch):
        ...
        for i in movable_instructions:
            if i.overwritten != "out":
                ...
                self.patch_bin(i.address, nop_opcode * len(i.bytes))
